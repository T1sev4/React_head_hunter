...
export default function MyApplies(){
  ...
  console.log(applies);
    ...
    let sortedApplies = [...applies].sort((a, b) => {
      let aPart, bPart;
      if(sortKey === 'status'){
        aPart = a.status
        bPart = b.status
    ...
    <div className="table">
      <div className="row row-header flex">
        <div className={`col ${sortDirection}`} onClick={() => sortBy('status')}>
          Статус {sortKey === 'status' && <img src="/images/angle-right.svg" alt="arrotw right" />}
        </div>
        <div className={`col ${sortDirection}`} onClick={() => sortBy('vacancy')}>
          Вакансия {sortKey === 'vacancy' && <img src="/images/angle-right.svg" alt="arrotw right" />}
        </div>
        <div className={`col ${sortDirection}`} onClick={() => sortBy('updatedAt')}>
          Дата {sortKey === 'updatedAt' && <img src="/images/angle-right.svg" alt="arrotw right" />}
        </div>
      </div>
      {showApplies}
    </div>
